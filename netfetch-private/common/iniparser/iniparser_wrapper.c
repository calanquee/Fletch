...
int IniparserWrapper::get_path_resolution_offset() {
    int tmp = iniparser_getint(ini, "global:path_resolution_offset", -1);
    if (tmp == -1) {
        printf("Invalid entry of [global:path_resolution_offset]: %d\n", tmp);
        exit(-1);
    }
    return tmp;
}
...
int IniparserWrapper::get_file_metadata_length() {
    int tmp = iniparser_getint(ini, "global:file_metadata_length", -1);
    if (tmp == -1) {
        printf("Invalid entry of [global:file_metadata_length]: %d\n", tmp);
        exit(-1);
    }
    return tmp;
}
...
int IniparserWrapper::get_dir_metadata_length() {
    int tmp = iniparser_getint(ini, "global:dir_metadata_length", -1);
    if (tmp == -1) {
        printf("Invalid entry of [global:dir_metadata_length]: %d\n", tmp);
        exit(-1);
    }
    return tmp;
}
...
short IniparserWrapper::get_client_port1_for_handling_updatecache() {
    int tmp = iniparser_getint(ini, "client:client_port1_for_handling_updatecache", -1);
    if (tmp == -1) {
        printf("Invalid entry of [client:client_port1_for_handling_updatecache]: %d\n", tmp);
        exit(-1);
    }
    return short(tmp);
}

short IniparserWrapper::get_client_port2_for_handling_updatecache() {
    int tmp = iniparser_getint(ini, "client:client_port2_for_handling_updatecache", -1);
    if (tmp == -1) {
        printf("Invalid entry of [client:client_port2_for_handling_updatecache]: %d\n", tmp);
        exit(-1);
    }
    return short(tmp);
}
...
// Qingxiu: add an interface to get file parser offset
uint16_t IniparserWrapper::get_file_parser_offset() {
    int tmp = iniparser_getint(ini, "server:file_parser_offset", -1);
    if (tmp == -1) {
        printf("Invalid entry of [server:file_parser_offset]: %d\n", tmp);
        exit(-1);
    }
    return uint16_t(tmp);
}
...
int IniparserWrapper::get_server_thread_pool_size(){
    int tmp = iniparser_getint(ini, "server:server_thread_pool_size", 0);
    if (tmp == -1) {
        printf("Invalid entry of [server:server_thread_pool_size]: %d\n", tmp);
        exit(-1);
    }
    return tmp;
}

int IniparserWrapper::get_delete_server_number(){
    int tmp = iniparser_getint(ini, "server:delete_server_number", 1);
    if (tmp == -1) {
        printf("Invalid entry of [server:delete_server_number]: %d\n", tmp);
        exit(-1);
    }
    return tmp;
}
...
const char* IniparserWrapper::get_server_ip_for_invalidation_list(uint32_t server_physical_idx) {
    char key[256];
    sprintf(key, "server%u:server_ip_for_invalidation_list", server_physical_idx);
    const char* server_ip_for_invalidation_list = iniparser_getstring(ini, key, nullptr);
    if (server_ip_for_invalidation_list == nullptr) {
        printf("Invalid entry of [%s]\n", key);
        exit(-1);
    }
    return server_ip_for_invalidation_list;
}

const char* IniparserWrapper::get_another_server_ip_for_invalidation_list(uint32_t server_physical_idx) {
    char key[256];
    sprintf(key, "server%u:another_server_ip_for_invalidation_list", server_physical_idx);
    const char* another_server_ip_for_invalidation_list = iniparser_getstring(ini, key, nullptr);
    if (another_server_ip_for_invalidation_list == nullptr) {
        printf("Invalid entry of [%s]\n", key);
        exit(-1);
    }
    return another_server_ip_for_invalidation_list;
}

const char* IniparserWrapper::get_server_port_for_invalidation_list(uint32_t server_physical_idx) {
    char key[256];
    sprintf(key, "server%u:server_port_for_invalidation_list", server_physical_idx);
    const char* server_port_for_invalidation_list = iniparser_getstring(ini, key, nullptr);
    if (server_port_for_invalidation_list == nullptr) {
        printf("Invalid entry of [%s]\n", key);
        exit(-1);
    }
    return server_port_for_invalidation_list;
}
...
uint32_t IniparserWrapper::get_cache_lookup_table_size(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:cache_lookup_table_size", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:cache_lookup_table_size", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
        printf("Invalid entry of [switch:cache_lookup_table_size]: %d\n", tmp);
        exit(-1);
    }
    return uint32_t(tmp);
}
...
const char* IniparserWrapper::get_root_path_for_fs_tree(uint32_t switchos_idx) {
    const char* root_path_for_fs_tree;
    if (switchos_idx == 0x3f3f3f3f) {
        root_path_for_fs_tree = iniparser_getstring(ini, "switch:root_path_for_fs_tree", nullptr);
    } else {
        char key[1024];
        sprintf(key, "switch%u:root_path_for_fs_tree", switchos_idx);
        root_path_for_fs_tree = iniparser_getstring(ini, key, nullptr);
    }
    if (root_path_for_fs_tree == nullptr) {
        printf("Invalid entry of [switch:root_path_for_fs_tree]\n");
        exit(-1);
    }
    return root_path_for_fs_tree;
}
...
short IniparserWrapper::get_switchos_server_eviction_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_server_eviction_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_server_eviction_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
        printf("Invalid entry of [switch:switchos_server_eviction_port]: %d\n", tmp);
        exit(-1);
    }
    return short(tmp);
}

// Qingxiu: add an interface to get switchos evict threshold
uint16_t IniparserWrapper::get_switchos_evict_threshold(uint32_t switchos_idx){
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_evict_threshold", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_evict_threshold", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
        printf("Invalid entry of [switch:switchos_evict_threshold]: %d\n", tmp);
        exit(-1);
    }
    return uint16_t(tmp);
}
...
// Qingxiu: obtain load freq. port from config
short IniparserWrapper::get_reflector_cp2dpserver_loadfreq_port(uint32_t reflector_idx) {
    int tmp;
    if (reflector_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "reflector:reflector_cp2dpserver_port_for_loadfreq", -1);
    } else {
        char key[256];
        sprintf(key, "reflector%u:reflector_cp2dpserver_port_for_loadfreq", reflector_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
        printf("Invalid entry of [reflector:reflector_cp2dpserver_port_for_loadfreq]: %d\n", tmp);
        exit(-1);
    }
    return short(tmp);
}
...
short IniparserWrapper::get_server_valuedeleteserver_port_start() {
    int tmp = iniparser_getint(ini, "server:server_valuedeleteserver_port_start", -1);
    if (tmp == -1) {
        printf("Invalid entry of [server:server_valuedeleteserver_port_start]: %d\n", tmp);
        exit(-1);
    }
    return short(tmp);
}
