...
enum class PacketType {
    ...
    DELREQ_SEQ_CASE3_BEINGEVICTED_SPINE = 0x00b2,
    ...
    BACKUP = 0x009f,
    BACKUPACK = 0x109f,
    CACHE_EVICT_LOADFREQ_INSWITCH_PERIOD = 0x0084,
    DELETE_SWITCHOS = 0x00f2,
};
/*enum class PacketType {
    GETREQ, , DELREQ, SCANREQ, GETRES, PUTRES, DELRES, SCANRES_SPLIT, GETREQ_INSWITCH, GETREQ_POP, GETREQ_NLATEST,
...
template <class key_t, class val_t>
class PutRequestSeq : public Packet<key_t> {  // ophdr 2+16 + val 2+128 +  2 bytes + seq 4 bytes + (snapshot token 4 bytes, FarReach Only)
public:
    PutRequestSeq();
...
public:
    ...
    CachePopInswitch(method_t methodid, key_t key, val_t val, uint32_t seq, uint16_t freeidx, bool stat, uint16_t bitmap, uint16_t regidx, bool isfile);
