...
uint32_t Packet<key_t>::get_ophdrsize(method_t methodid) {
    ...
    case FARREACH_ID:
        ...
        break;
    case NOCACHE_ID:
        result = sizeof(optype_t) + sizeof(keydepth_t);
        break;
    case NETCACHE_ID:
        result = sizeof(optype_t) + sizeof(keydepth_t);
        break;
...
bool Packet<key_t>::is_singleswitch(method_t methodid) {
    if (methodid == FARREACH_ID || methodid == NOCACHE_ID || methodid == NETCACHE_ID || methodid == NETFETCH_ID) {
        return true;
    }
    return false;
}
...
template <class key_t>
Packet<key_t>::Packet()
    : _methodid(INVALID_ID), _type(static_cast<optype_t>(0)), _key(key_t::min()), _globalswitchidx(0), _spineswitchidx(0), _leafswitchidx(0), _bitmap(0), _regidx(0) {
}

template <class key_t>
Packet<key_t>::Packet(method_t methodid, packet_type_t type, key_t key)
    : _methodid(methodid), _type(static_cast<optype_t>(type)), _key(key), _globalswitchidx(0), _spineswitchidx(0), _leafswitchidx(0), _bitmap(0), _regidx(0) {
}

template <class key_t>
Packet<key_t>::Packet(method_t methodid, packet_type_t type, switchidx_t globalswitchidx, key_t key)
    : _methodid(methodid), _type(static_cast<optype_t>(type)), _globalswitchidx(globalswitchidx), _key(key), _spineswitchidx(0), _leafswitchidx(0), _bitmap(0), _regidx(0) {
}
...
uint32_t Packet<key_t>::serialize_ophdr(char* const data, uint32_t max_size) {
    ...
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_DEBUG] [line %d] call serialize_ophdr\n", __LINE__);
    // #endif
    uint32_t ophdr_size = Packet<key_t>::get_ophdrsize(this->_methodid);  // ophdr_size = 4
    ...
    serialize_keydepth((uint16_t)1, begin, max_size);
    begin += sizeof(keydepth_t);
...
uint32_t Packet<key_t>::deserialize_ophdr(const char* data, uint32_t recv_size) {
    ...
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_DEBUG] ophdr_size: %d, recv_size: %d\n", ophdr_size, recv_size);
    // #endif
    ...
    begin += tmp_typesize;
    // DBGprint("deserialize_ophdr recv_size %d\n", recv_size);
    // Qingxiu: we do not go into the following if
    if (!Packet<key_t>::is_singleswitch(this->_methodid)) {
        if (this->_methodid == DISTCACHE_ID) {
    ...
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_DEBUG] tmp_keysize: %d\n", tmp_keysize);
    // #endif
    begin += tmp_keysize;
    return uint32_t(begin - data);
}
...
#ifdef NETFETCH_DEBUG
    printf("[NETFETCH_DEBUG] [line %d] call GetRequest deserialize\n", __LINE__);
#endif
...
void GetRequest<key_t>::deserialize(const char* data, uint32_t recv_size) {
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_DEBUG] [line %d] call deserialize\n", __LINE__);
    // #endif
    uint32_t my_size = this->size();
...
uint32_t GetResponse<key_t, val_t>::serialize(char* const data, uint32_t max_size) {
    ...
    // INVARIANT(max_size >= my_size);
#ifdef NETFETCH_DEBUG
    printf("[NETFETCH_DEBUG] [line %d] call GetResponse serialize\n", __LINE__);
#endif
    char* begin = data;
    uint32_t tmp_ophdrsize = this->serialize_ophdr(begin, max_size);
#ifdef NETFETCH_DEBUG
    printf("[NETFETCH_DEBUG] [line %d] tmp_ophdrsize: %d\n", tmp_ophdrsize, __LINE__);
#endif
    begin += tmp_ophdrsize;
    uint32_t tmp_valsize = this->_val.serialize(begin, max_size - uint32_t(begin - data));
    begin += tmp_valsize;
...
#ifdef NETFETCH_DEBUG
    printf("[NETFETCH_DEBUG] GetResponseServer<key_t, val_t>::GetResponseServer\n");
#endif
...
uint32_t GetResponseSeq<key_t, val_t>::serialize(char* const data, uint32_t max_size) {
    ...
    // INVARIANT(max_size >= my_size);
#ifdef NETFETCH_DEBUG
    printf("[NETFETCH_DEBUG] call GetResponseSeq serialize\n");
#endif
    char* begin = data;
    uint32_t tmp_ophdrsize = this->serialize_ophdr(begin, max_size);
    begin += tmp_ophdrsize;
...
#ifdef NETFETCH_DEBUG
    printf("[NETFETCH_DEBUG] call GetResponseServerSeq constructor\n");
#endif
...
void CachePop<key_t, val_t>::deserialize(const char* data, uint32_t recv_size) {
    ...
    uint32_t tmp_ophdrsize = this->deserialize_ophdr(begin, recv_size);
    begin += tmp_ophdrsize;
    if (recv_size - uint32_t(begin - data) <= 0) {
        // dump buffer
...
// for netfetch
template <class key_t, class val_t>
CachePopInswitch<key_t, val_t>::CachePopInswitch(method_t methodid, key_t key, val_t val, uint32_t seq, uint16_t freeidx, bool stat, uint16_t bitmap, uint16_t regidx, bool isfile)
    : PutRequestSeq<key_t, val_t>(methodid, key, val, seq), _freeidx(freeidx), _stat(stat) {
    INVARIANT(methodid == NETFETCH_ID);
    this->_type = static_cast<optype_t>(PacketType::CACHE_POP_INSWITCH);
    this->_bitmap = bitmap;
    this->_regidx = regidx;
    this->_isfile = isfile;
...
uint32_t CachePopInswitch<key_t, val_t>::serialize(char* const data, uint32_t max_size) {
    ...
    uint32_t tmp_ophdrsize = this->serialize_ophdr(begin, max_size);
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_POP_INSWITCH] [line %d] tmp_ophdrsize: %d\n", __LINE__, tmp_ophdrsize);
    // #endif
    begin += tmp_ophdrsize;
    uint32_t tmp_valsize = this->_val.serialize(begin, max_size - uint32_t(begin - data));
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_POP_INSWITCH] [line %d] tmp_valsize: %d\n", __LINE__, tmp_valsize);
    // #endif
    begin += tmp_valsize;
    uint32_t tmp_shadowtypesize = serialize_packet_type(this->_type, begin, max_size - uint32_t(begin - data));  // shadowtype
                                                                                                                 // #ifdef NETFETCH_DEBUG
                                                                                                                 //     printf("[NETFETCH_POP_INSWITCH] [line %d] tmp_shadowtypesize: %d\n", __LINE__, tmp_shadowtypesize);
                                                                                                                 //     printf("[NETFETCH_POP_INSWITCH] [line %d] shadowtype: %d\n", __LINE__, this->_type);
                                                                                                                 // #endif
    begin += tmp_shadowtypesize;
    uint32_t bigendian_seq = htonl(this->_seq);
    memcpy(begin, (void*)&bigendian_seq, sizeof(uint32_t));  // little-endian to big-endian
    ...
    int tmp_inswitch_prev_bytes = Packet<key_t>::get_inswitch_prev_bytes(this->_methodid);
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_POP_INSWITCH] [line %d] tmp_inswitch_prev_bytes: %d\n", __LINE__, tmp_inswitch_prev_bytes);
    // #endif
    // Qingxiu: copy bitmap to packet
    if (this->_methodid == NETFETCH_ID) {
        memset(begin, 0, tmp_inswitch_prev_bytes - 5);
        begin += tmp_inswitch_prev_bytes - 5;
        memcpy(begin, (void*)&this->_isfile, sizeof(bool));
        begin += sizeof(bool);
        uint16_t idx_for_latest_deleted_frequency_reg = htons(uint16_t(this->_freeidx));
        memcpy(begin, (void*)&idx_for_latest_deleted_frequency_reg, sizeof(uint16_t));
        begin += sizeof(uint16_t);  // idx_for_latest_deleted_frequency_reg
        uint16_t bigendian_bitmap = htons(uint16_t(this->_bitmap));
        memcpy(begin, (void*)&bigendian_bitmap, sizeof(uint16_t));
        begin += sizeof(uint16_t);
        uint16_t bigendian_freeidx = htons(uint16_t(this->_regidx));  // for val regs
                                                                      // #ifdef NETFETCH_DEBUG
                                                                      //         printf("[NETFETCH_POP_INSWITCH] [line %d] freeidx: %d\n", __LINE__, this->_regidx);
                                                                      // #endif
...
// Qingxiu: for netfetch
template <class key_t, class val_t>
uint16_t CachePopInswitch<key_t, val_t>::bitmap() const {
    return _bitmap;
}

template <class key_t, class val_t>
uint16_t CachePopInswitch<key_t, val_t>::regidx() const {
    return _regidx;
}

template <class key_t, class val_t>
bool CachePopInswitch<key_t, val_t>::isfile() const {
    return _isfile;
}
...
uint32_t CacheEvictLoadfreqInswitch<key_t>::serialize(char* const data, uint32_t max_size) {
    ...
    int tmp_inswitch_prev_bytes = Packet<key_t>::get_inswitch_prev_bytes(this->_methodid);
    if (this->_methodid == NETFETCH_ID) {
        memset(begin, 0, tmp_inswitch_prev_bytes - 4);
        begin += tmp_inswitch_prev_bytes - 4;
        uint16_t bigendian_evictidx = htons(uint16_t(this->_evictidx));
        memcpy(begin, (void*)&bigendian_evictidx, sizeof(uint16_t));  // write idx_for_latest_regs
        begin += sizeof(uint16_t);
        memset(begin, 0, 4);
        begin += sizeof(uint16_t) + sizeof(uint16_t);
    } else {
        memset(begin, 0, tmp_inswitch_prev_bytes);  // the first bytes of inswitch_hdr
        begin += tmp_inswitch_prev_bytes;
        uint16_t bigendian_evictidx = htons(uint16_t(this->_evictidx));
...
CacheEvictLoadfreqInswitchAck<key_t>::CacheEvictLoadfreqInswitchAck(method_t methodid, const char* data, uint32_t recv_size) {
    this->_methodid = methodid;
    this->deserialize(data, recv_size);
    if (!(this->_type == optype_t(packet_type_t::CACHE_EVICT_LOADFREQ_INSWITCH_ACK) || this->_type == optype_t(packet_type_t::CACHE_EVICT_LOADFREQ_INSWITCH_PERIOD_ACK))) {
        for (int i = 0; i < recv_size; i++) {
            printf("%02x ", data[i]);
        }
    }
    INVARIANT(this->_type == optype_t(packet_type_t::CACHE_EVICT_LOADFREQ_INSWITCH_ACK) || this->_type == optype_t(packet_type_t::CACHE_EVICT_LOADFREQ_INSWITCH_PERIOD_ACK));
...
uint32_t SetvalidInswitch<key_t>::serialize(char* const data, uint32_t max_size) {
    ...
    uint32_t tmp_ophdrsize = this->serialize_ophdr(begin, max_size);
    begin += tmp_ophdrsize;
    // if (this->_methodid == NETCACHE_ID || this->_methodid == NETFETCH_ID) {
    //     keydepth_t keydepth = htons(1);
    //     memcpy(begin, (void*)&keydepth, sizeof(keydepth_t));  // little-endian to big-endian
    //     begin += sizeof(keydepth_t);
    // }
    uint32_t tmp_shadowtypesize = serialize_packet_type(this->_type, begin, max_size - uint32_t(begin - data));  // shadowtype
    begin += tmp_shadowtypesize;
...
{
    INVARIANT(methodid == NETCACHE_ID || methodid == DISTCACHE_ID || methodid == NETFETCH_ID);
...
NetcacheCachePop<key_t>::NetcacheCachePop(method_t methodid, const char* data, uint32_t recv_size) {
    INVARIANT(methodid == NETCACHE_ID || methodid == DISTCACHE_ID || methodid == NETFETCH_ID);
    this->_methodid = methodid;
    // DBGprint("NetcacheCachePop recv_size %d\n", recv_size);
    this->deserialize(data, recv_size);
    INVARIANT(static_cast<packet_type_t>(this->_type) == PacketType::NETCACHE_CACHE_POP || static_cast<packet_type_t>(this->_type) == PacketType::NETCACHE_PRE_CACHE_POP);
    INVARIANT(this->_serveridx >= 0);
}
...
void NetcacheCachePop<key_t>::deserialize(const char* data, uint32_t recv_size) {
    ...
    // // INVARIANT(my_size == recv_size);
    // // Qingxiu: rewrite
    // if (my_size != recv_size) {
    //     recv_size = my_size;
    // }
    // DBGprint("NetcacheCachePop deserialize recv_size %d\n", recv_size);
#ifdef NETFETCH_DBEUG
    printf("[line %d] NetcacheCachePop deserialize recv_size %d\n", __LINE__, recv_size);
#endif
    const char* begin = data;
    uint32_t tmp_ophdrsize = this->deserialize_ophdr(begin, recv_size);
    begin += tmp_ophdrsize;
...
NetcacheCachePopFinish<key_t>::NetcacheCachePopFinish(method_t methodid, key_t key, uint16_t serveridx)
    ...
    INVARIANT(methodid == NETCACHE_ID || methodid == NETFETCH_ID);
...
template <class key_t>
uint32_t NetcacheWarmupRequestInswitchPop<key_t>::size() {  // netfetch use
    ...
    if (this->_methodid == NETFETCH_ID) {
        return Packet<key_t>::get_ophdrsize(this->_methodid) + sizeof(netreach_key_t) + Packet<key_t>::get_inswitch_prev_bytes(this->_methodid) + sizeof(uint16_t) + sizeof(optype_t) + Packet<key_t>::get_clone_bytes(this->_methodid);
    }
}
...
void NetcacheWarmupRequestInswitchPop<key_t>::deserialize(const char* data, uint32_t recv_size) {
    uint32_t my_size = this->size();
    // if(my_size > recv_size) {
    //     printf("[NetcacheWarmupRequestInswitchPop] WARN: my_size %d > recv_size %d\n", my_size, recv_size);
    //     // dump buffer
    //     for (int i = 0; i < recv_size; i++) {
    //         printf("%02x ", data[i]);
    //     }
...
static uint32_t serialize_packet_type(optype_t type, char* data, uint32_t maxsize) {
    ...
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_DEBUG] [line %d] call serialize_packet_type() in packet_format_impl.h\n", __LINE__);
    // #endif
    // Qingxiu: modify from uint16_t to uint32_t
    optype_t bigendian_type = htons(type);
    // // Qingxiu: modify the last two bytes of bigendian_type to 01 01
    // uint16_t keydepth = 0x0101;
    // bigendian_type = (bigendian_type & 0x0000FFFF) | keydepth;
    // // Qingxiu: print the type in hex
    // for(int i=0;i<4;++i){
    //     printf("%02x ", ((char*)&bigendian_type)[i]);
    // }
    // printf("\n");
    // uint16_t bigendian_keydepth = htons(keydepth);
...
static uint32_t serialize_keydepth(keydepth_t depth, char* data, uint32_t maxsize) {
    INVARIANT(maxsize >= sizeof(optype_t));
    // #ifdef NETFETCH_DEBUG
    //     printf("[NETFETCH_DEBUG] [line %d] call serialize_keydepth() in packet_format_impl.h\n", __LINE__);
    // #endif
    keydepth_t bigendian_type = htons(depth);
    memcpy(data, (void*)&bigendian_type, sizeof(optype_t));

    return sizeof(keydepth_t);
}
...
static packet_type_t get_packet_type(const char* data, uint32_t recv_size) {
    ...
    // printf("[NETFETCH_DBEUG] call get_packet_type() in packet_format_impl.h\n");
    optype_t tmp;
    memcpy((void*)&tmp, data, sizeof(optype_t));
    // printf("[NETFETCH_DEBUG] sizeof(optype_t): %d\n", sizeof(optype_t));
    tmp = ntohs(tmp);
    return static_cast<packet_type_t>(tmp);
}

static keydepth_t get_keydepth(const char* data, uint32_t recv_size) {
    INVARIANT(recv_size >= sizeof(optype_t) + sizeof(keydepth_t));
    keydepth_t tmp;
    memcpy((void*)&tmp, data + sizeof(optype_t), sizeof(keydepth_t));
    tmp = ntohs(tmp) & 0x00FF;
    return tmp;
}

static keydepth_t get_real_keydepth(const char* data, uint32_t recv_size) {
    INVARIANT(recv_size >= sizeof(optype_t) + sizeof(keydepth_t));
    keydepth_t tmp;
    memcpy((void*)&tmp, data + sizeof(optype_t), sizeof(keydepth_t));
    tmp = tmp & 0x00FF;
...
static uint16_t get_packet_clientlogicalidx(method_t methodid, const char* data, uint32_t recvsize) {
...
#ifdef NETFETCH_DEBUG
    printf("[NETFETCH_DEBUG] [line %d] recvsize: %u\n", __LINE__, recvsize);
    printf("[NETFETCH_DEBUG] [line %d] tmp_ophdrsize: %u\n", __LINE__, tmp_ophdrsize);
    printf("[NETFETCH_DEBUG] [line %d] sizeof(uint16_t): %lu\n", __LINE__, sizeof(uint16_t));
#endif
    ...
    if (tmp_optype == packet_type_t::PUTREQ_LARGEVALUE) {
        ...
        prevbytes = tmp_ophdrsize;  // op_hdr
#ifdef NETFETCH_DEBUG
        printf("[NETFETCH_DEBUG] [line %d] prevbytes: %u\n", __LINE__, prevbytes);
#endif
    } else {
