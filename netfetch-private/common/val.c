...
uint32_t Val::deserialize_val(const char *buf, uint32_t buflen) {
	...
	if (val_length > 0) {
		...
		if(!(deserialize_size >= 0 && buflen >= deserialize_size)){
			printf("deserialize_size: %d, buflen: %d\n", deserialize_size, buflen);
			printf("val_length: %d, padding_size: %d\n", val_length, padding_size);
			// dump buf
			printf("buf: ");
