...
parser nocahceParser(packet_in packet,
	...
	state parse_udp_srcport {
		transition select(hdr.udp_hdr.srcPort) {
			...
			5011: parse_keydepth; // Qingxiu: add port 5011 here since we add one port for loading freq.
	...
	state parse_keydepth{
		...
		transition select(hdr.op_hdr.keydepth){
			// Qingxiu: depth plus 256 to avoid 00 in the packet 
			1: parse_key1;
			2: parse_key2;
			3: parse_key3;
			4: parse_key4;
