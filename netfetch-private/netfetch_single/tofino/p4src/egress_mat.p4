...
#define clone_e2e(mir_ses)                      \
	...
	apply{
		...
		if (hdr.key1_hdr.isValid()) {
			...
			if(hdr.op_hdr.keydepth == hdr.op_hdr.real_keydepth){
				hdr.seq_hdr.unlock_keyindex = hdr.key2_hdr.keyhihihi;
			}else{
				hdr.seq_hdr.unlock_keyindex = hdr.shadowtype_hdr.shadowtype;
			}
			// stage 2
