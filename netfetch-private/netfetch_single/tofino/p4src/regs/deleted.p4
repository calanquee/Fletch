Register<bit<1>,bit<32>>(KV_BUCKET_COUNT*3) deleted_reg;
...
action get_deleted() {
	meta.is_deleted = deleted_reg_get_alu.execute((bit<32>)hdr.inswitch_hdr.idx_for_latest_deleted_frequency_reg);
}

action set_and_get_deleted() {
	meta.is_deleted = deleted_reg_get_set_alu.execute((bit<32>)hdr.inswitch_hdr.idx_for_latest_deleted_frequency_reg);
}


action reset_and_get_deleted() {
	meta.is_deleted =deleted_reg_get_reset_alu.execute((bit<32>)hdr.inswitch_hdr.idx_for_latest_deleted_frequency_reg);
}
