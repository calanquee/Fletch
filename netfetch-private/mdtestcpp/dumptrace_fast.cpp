...
std::unordered_map<int, int> fast_counter_nodes_load(const std::string& access_pattern, const std::unordered_map<int, int>& countmap) {
        ...
        for (size_t i = 0; i < lines.size(); ++i) {
            ...
            if (!line.empty()) {
                ...
                if (ext != -1 && countmap.find(ext) != countmap.end()) {
                    id_name[ext] = line;
                    auto md5_value = computeMD5(line);
