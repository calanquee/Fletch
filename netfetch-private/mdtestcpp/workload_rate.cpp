...
unsigned long long folders_per_depth[10];
unsigned long long file_strides[10];
unsigned long long folder_strides[10];
...
unsigned long long getParentDir(unsigned long long item_num, int items_per_dir) {
    ...
    // return early
    // return item_num / items_per_dir;
    int idx = 0;
    for(idx = 0; idx < 10; idx++) {
        if (item_num < file_strides[idx]) {
        break;
        }
    }

    //   printf("getParentDir: item_num = %llu, idx = %d\n", item_num, idx);
    //   printf("getParentDir: file_strides[%d] = %llu, folders_per_depth[%d] = %llu, folder_strides[%d] = %llu\n",
    //          idx-1, file_strides[idx-1], idx, folders_per_depth[idx], idx-1, folder_strides[idx-1]);

    unsigned long long parent_dir = (item_num - file_strides[idx-1]) / folders_per_depth[idx] + folder_strides[idx-1];
    
    //   printf("getParentDir: parent_dir = %llu\n", parent_dir);
    return parent_dir;
}
...
void generate_op_array(int total_ops) {
    ...
    for (const auto &[op, ratio] : operationRatios) {
        ...
        if(op == READDIR) {
            printf("[Line %d] READDIR ratio: %f, count: %d\n", __LINE__, ratio, count);
        }
...
void generate_reqs_mixed_by_rand_array(const std::string &path, std::string &base_tree_name, std::string &stat_name, int branch_factor, int items_per_dir) {
     printf("[Line %d rand_array size: %zu]\n", __LINE__, rand_array.size());
    int parent_dir = 0, item_num = 0;
    ...
    for (int i = 0; i < rand_array_size; i++) {
        ...
        // printf("item_num: %d, stat_name: %s\n", rand_array[i], stat_name.c_str());
        parent_dir = getParentDir(rand_array[i], items_per_dir);
        // printf("parent_dir: %d\n", parent_dir);
        if (parent_dir > 0) {
            // Construct directory tree based on parent_dir
            item = std::string(base_tree_name) + "." + std::to_string(parent_dir) + "/" + item;
        ...
        std::string fullPath = path + "/" + item;
        // printf("[Line %d] fullPath: %s\n", __LINE__, fullPath.c_str());
        // check partition
        auto hashValue = computeMD5(fullPath);
        uint32_t nodeidx = mapToNameNodeidx(hashValue);
        if (nodeidx != bottleneck_id && nodeidx != rotation_id && mode == 1) {
...
std::string gen_file_name(const std::string &path, int idx) {
    ...
    int parent_dir = getParentDir(idx, items_per_dir);
...
// ./mdtest -a 64 -n 31981446 -m -z 8 -b 4 -d / -Y /home/jz/In-Switch-FS-Metadata/netfetch-private/mdtestcpp/workload/workloads/access_file_31981446_32000000_10_4_0.9_3.out -P 50 