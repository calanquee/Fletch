...
void create_remove_items_helper(int dirs, int create, const std::string &path, unsigned long long itemNum, unsigned long long items_per_dir) {
...
void create_remove_items_helper_old(int dirs, int create, const std::string &path, unsigned long long itemNum) {
    // printf("create_remove_items_helper %d\n",items_per_dir);
    for (unsigned long long i = 0; i < items_per_dir; i++) {
        std::string curr_item;
        if (dirs) {
...
unsigned long long getFileNum(unsigned long long currDir, int currDepth) {
  // from currDepth, we know how many files we have before.
  // then we need to compute how many folders before.
  return (currDir - folder_strides[currDepth - 1]) * folders_per_depth[currDepth] + file_strides[currDepth - 1];
}
    ...
    if (currDepth == 0) {
        ...
        if (!leaf_only || (depth == 0 && leaf_only)) {
            if (collective) {
                ...
                create_remove_items_helper(dirs, create, path, 0, folders_per_depth[currDepth]);

                // create_remove_items_helper_old(dirs, create, path, 0);
            }
        }
        ...
        for (int i = 0; i < branch_factor; i++) {
            ...
            if (!leaf_only || (leaf_only && currDepth == depth)) {
                if (collective) {
                    ...
                    create_remove_items_helper(dirs, create, new_path, getFileNum(currDir, currDepth), folders_per_depth[currDepth]);

                    // create_remove_items_helper_old(dirs, create, new_path, currDir * items_per_dir);
                }
            }
...
void mdtest_read(int random, int dirs, const std::string &path) {
    ...
    for (unsigned long long i = 0; i < stop; i++) {
        ...
        // unsigned long long parent_dir = item_num / items_per_dir;
        unsigned long long parent_dir = getParentDir(item_num, items_per_dir);
...
void mdtest_write_hot_paths_to_switch_for_netcache(std::string &path) {
    ...
    for (int i = 0; i < hot_paths_count; i++) {
        ...
        parent_dir = getParentDir(item_num, items_per_dir);
...
int main(int argc, char **argv) {
    ...
    printf("depth = %d, branch_factor = %d, num_dirs_in_tree = %llu\n",
           depth, branch_factor, num_dirs_in_tree);
    

    // base 4
    // float file_ratios[9] = {4.06875e-05, 0.00016275, 0.000651, 0.002604, 0.010416, 0.041664, 0.166656, 0.666624, 0.11098778125};
    // base 5
    float file_ratios[9] = {
        9.102424325931854e-06, 4.5512121629659267e-05, 0.00022756060814829634, 0.0011378030407414815, 0.005689015203707409, 0.028445076018537043, 0.1422253800926852, 0.7111269004634261, 0.11109365002679898

    };

    float sum = 0;
    for(int dp=0;dp <depth+1;dp++){
      sum += file_ratios[dp];
    }
    double file_ratios_sum = 0.0;
    for(int dp=0;dp <depth+1;dp++){
        file_ratios[dp] /= sum;
        file_ratios_sum += file_ratios[dp];
        printf("file_ratios[%d]: %f\n", dp, file_ratios[dp]);
    }
    printf("file_ratios_sum: %f\n", file_ratios_sum);
    int sum_files = 0;
    items = 32000000;
    for(int dp=0; dp<depth+1; dp++){
      int files_per_depth = items * file_ratios[dp];
      int folder_per_depth = pow(branch_factor, dp);
      folders_per_depth[dp] = files_per_depth / folder_per_depth;
      int left = files_per_depth - folders_per_depth[dp] * folder_per_depth;
      printf("depth %d files %d folders %d number %llu remain %d\n", dp, files_per_depth, folder_per_depth, folders_per_depth[dp], left);
      sum_files += files_per_depth;
    }
    printf("sum_files: %d\n", sum_files);
    for (int i = 0; i < depth+1; i++) {
        if (i == 0) {
          folder_strides[i] = 1;
          file_strides[i] = folders_per_depth[i];
        }
        else {
          folder_strides[i] = folder_strides[i-1] + pow(branch_factor, i);
          file_strides[i] = file_strides[i-1] + folders_per_depth[i] * pow(branch_factor, i);
        }
        printf("folder: %llu file: %llu \n", folder_strides[i], file_strides[i]);
    }
    // exit(0);
    // printf("num %lld \n", getFileNum(87380, 8));
    if (items_per_dir > 0) {
    ...
    items = file_strides[depth];
    printf("[LINE: %d] generating items: %lld \n", __LINE__, items);
    ...
    if (is_mixed == 1 && create_only == 1) {  // create + mkdir
        ...
        // if(rtries > 0) {
        //     printf("[MIXED][line %d] retries size:\n", __LINE__);
        // }
