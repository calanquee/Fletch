...
DIR_METADATA_SIZE = 22
FILE_METADATA_SIZE = 40
NETCACHE_METADATA_SIZE = 128

netfetch_cached_memory = 0
...
def handle_entry(filename, cache_capacity):
    global is_fs_tree_build, cached, uncached, netfetch_cached_memory
    is_stop = False
    capacity = compute_capacity_for_hot_report(filename)
    current_required_memory = (capacity-1)*DIR_METADATA_SIZE + FILE_METADATA_SIZE
    # if uncached[filename] >= HOT_REPORT_THRESHOLD*capacity*SCALE_COEFFICIENT:
    if uncached[filename] >= 0:
        if current_required_memory + netfetch_cached_memory > cache_capacity*NETCACHE_METADATA_SIZE:
            is_stop = True
            return is_stop
        netfetch_cached_memory += current_required_memory
