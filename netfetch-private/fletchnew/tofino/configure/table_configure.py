...
class TableConfigure(BfRuntimeTest):
    def setUp(self):
        ...
        bfrt_info = self.interface.bfrt_info_get("fletch")
        ...
        self.get_eport_for_getres_for_pkt_loss_tbl = bfrt_info.table_get("netcacheEgress.get_eport_for_getres_for_pkt_loss_tbl")
        ...
        self.access_counter_tbl = bfrt_info.table_get("netcacheIngress.access_counter_tbl")
        self.check_counter_tbl = bfrt_info.table_get("netcacheIngress.check_counter_tbl")
        ...
        self.access_counter_tbl.info.key_field_annotation_add("hdr.ipv4_hdr.srcAddr","ipv4")
        self.set_requested_uid_gid_tbl.info.key_field_annotation_add("hdr.ipv4_hdr.srcAddr","ipv4")
        self.get_eport_for_getres_for_pkt_loss_tbl.info.key_field_annotation_add("hdr.ipv4_hdr.srcAddr","ipv4")
        ...
        self.update_ipmac_srcport_tbl.info.data_field_annotation_add("client_mac", "netcacheEgress.update_ipmac_srcport_dstport_client2server", "mac")
        self.update_ipmac_srcport_tbl.info.data_field_annotation_add("server_mac", "netcacheEgress.update_ipmac_srcport_dstport_client2server", "mac")
        self.update_ipmac_srcport_tbl.info.data_field_annotation_add("client_ip", "netcacheEgress.update_ipmac_srcport_dstport_client2server", "ipv4")
        self.update_ipmac_srcport_tbl.info.data_field_annotation_add("server_ip", "netcacheEgress.update_ipmac_srcport_dstport_client2server", "ipv4")
    ...
    def configure_access_counter_tbl(self):
        print(server_ips)
        server_index = 1
        for tmp_server_physical_idx in range(server_physical_num):
            ipv4addr = server_ips[tmp_server_physical_idx]
            if server_index == 1:
                for port in range(1280, 1300):
                    tmp_counter_index = port - 1280 # [0, 19]
                    key = self.access_counter_tbl.make_key(
                        [gc.KeyTuple('hdr.op_hdr.optype', PRE_GETRES),
                        gc.KeyTuple('hdr.ipv4_hdr.srcAddr', ipv4addr, prefix_len=32),
                        gc.KeyTuple('hdr.udp_hdr.srcPort', port)])
                    data = self.access_counter_tbl.make_data(
                        [gc.DataTuple('counter_index', tmp_counter_index)],
                        'netcacheIngress.get_counter_server{}'.format(server_index))
                    self.access_counter_tbl.entry_add(self.target, [key], [data])
            if server_index == 2:
                for port in range(1280, 1300):
                    tmp_counter_index = port - 1260 # [20, 39]
                    key = self.access_counter_tbl.make_key(
                        [gc.KeyTuple('hdr.op_hdr.optype', PRE_GETRES),
                        gc.KeyTuple('hdr.ipv4_hdr.srcAddr', ipv4addr, prefix_len=32),
                        gc.KeyTuple('hdr.udp_hdr.srcPort', port)])
        ...
        for tmp_server_physical_idx in range(server_physical_num):
            ...
            if server_index == 1:
                for port in range(1280, 1300):
                    tmp_counter_index = port - 1280 # [0, 19]
                    key = self.access_counter_tbl.make_key(
                        [gc.KeyTuple('hdr.op_hdr.optype', GETRES_UNLOCK),
                        gc.KeyTuple('hdr.ipv4_hdr.srcAddr', ipv4addr, prefix_len=32),
                        gc.KeyTuple('hdr.udp_hdr.srcPort', port)])
                    data = self.access_counter_tbl.make_data(
                        [gc.DataTuple('counter_index', tmp_counter_index)],
                        'netcacheIngress.update_counter_server{}'.format(server_index))
                    self.access_counter_tbl.entry_add(self.target, [key], [data])
            if server_index == 2:
                for port in range(1280, 1300):
                    tmp_counter_index = port - 1260 # [20, 39]
    ...
    def configure_check_counter_tbl(self):
        for i in range(256):
            key = self.check_counter_tbl.make_key(
                [gc.KeyTuple('hdr.op_hdr.optype', PRE_GETRES),
                gc.KeyTuple('meta.is_counter', i),
                gc.KeyTuple('meta.keyhihihi', i)])
            data = self.check_counter_tbl.make_data(
                [], 'netcacheIngress.set_counter_equal')
            self.check_counter_tbl.entry_add(self.target, [key], [data])
    ...
    def configure_get_eport_for_getres_for_pkt_loss_tbl(self):
        keys = []
        datas = []
        for tmp_optype in [GETRES_NO_UNLOCK, GETRES_FINAL]:
            for i in range(server_physical_num):
                server_index = i + 1
                keys.append(self.get_eport_for_getres_for_pkt_loss_tbl.make_key(
                        [gc.KeyTuple('hdr.op_hdr.optype', tmp_optype),
                        gc.KeyTuple('hdr.ipv4_hdr.srcAddr', server_ips[i], prefix_len=32)]))
                datas.append(self.get_eport_for_getres_for_pkt_loss_tbl.make_data(
                    [],
                    'netcacheEgress.res_from_server{}'.format(server_index)))
        self.get_eport_for_getres_for_pkt_loss_tbl.entry_add(self.target, keys, datas)
    ...
    def configure_ig_port_forward_tbl(self):
        ...
        datas = []
        is_acquire_lock_set = [0, 1]
        nolock_recir_port = recir_ports[self.nolock_pipe] # pipe0
        for is_acquire_lock in is_acquire_lock_set:
            keys.append(self.ig_port_forward_tbl.make_key(
                    ...
                    gc.KeyTuple('meta.is_counter_equal_to_sequence', 0)
                    ]))
        ...
        # for PRE_GETRES --> GETRES_UNLOCK (no pkt loss)
        keys.append(self.ig_port_forward_tbl.make_key(
                [gc.KeyTuple('hdr.op_hdr.optype', PRE_GETRES),
                gc.KeyTuple('meta.is_acquire_lock', 0),
                gc.KeyTuple('meta.is_counter_equal_to_sequence', 1)
                 ]))
        datas.append(self.ig_port_forward_tbl.make_data(
            [gc.DataTuple('recir_port', nolock_recir_port)],'netcacheIngress.recir_for_pre_getres'))
        
        # for PRE_GETRES --> GETRES_NO_UNLOCK (pkt loss happens)
        keys.append(self.ig_port_forward_tbl.make_key(
                [gc.KeyTuple('hdr.op_hdr.optype', PRE_GETRES),
                gc.KeyTuple('meta.is_acquire_lock', 0),
                gc.KeyTuple('meta.is_counter_equal_to_sequence', 0)
        ...
        datas.append(self.ig_port_forward_tbl.make_data(
            [gc.DataTuple('recir_port', nolock_recir_port)],'netcacheIngress.recir_for_pre_getres_wo_unlock'))
        
        # for GETRES_UNLOCK --> GETRES
        # keys.append(self.ig_port_forward_tbl.make_key(
        #         [gc.KeyTuple('hdr.op_hdr.optype', GETRES_UNLOCK),
        #         gc.KeyTuple('meta.is_acquire_lock', 0),
        #         gc.KeyTuple('meta.is_counter_equal_to_sequence', 0)
        #          ]))
        # datas.append(self.ig_port_forward_tbl.make_data(
        #     [],'netcacheIngress.update_getres_unlock_to_getres'))
    ...
    def configure_save_client_udpport_tbl(self):
        keys = []
        datas = []
        for tmpoptype in [GETREQ_INSWITCH, NETCACHE_WARMUPREQ_INSWITCH,SETVALID_INSWITCH,VALID_INSWITCH, GETRES_UNLOCK, GETRES_FINAL, GETRES_NO_UNLOCK]:
    ...
    def configure_access_lock_key_tbl(self):
        ...
        for lock_reg in range(2, lock_reg_num+2):
            ...
            for tmpoptype in [GETRES, GETRES_UNLOCK]: # unlock 2-9
                keys.append(access_lock_key_tbl.make_key([
                    gc.KeyTuple('hdr.op_hdr.optype', tmpoptype),
                    gc.KeyTuple('meta.is_cached_for_lock', 1),
                    gc.KeyTuple('hdr.op_hdr.keydepth', 1),
    ...
    def configure_get_last_key_tbl(self):
        ...
        for keydepth in range(1, 11):
            for tmpoptype in [GETREQ, GETREQ_RECIR, PRE_GETRES]:
    ...
    def configure_lastclone_lastscansplit_tbl(self):
        keys = []
        datas = []
        for tmpoptype in [NETCACHE_GETREQ_POP, NETCACHE_WARMUPREQ_INSWITCH_POP, GETRES_UNLOCK, GETRES_FINAL]:
    ...
    def configure_eg_port_forward_tbl(self):
        ...
        for(is_cached,is_hot,is_latest,is_deleted,tmp_client_sid,is_lastclone_for_pktloss,tmp_server_sid) in product(cached_list,hot_list,latest_list,deleted_list,tmp_client_sids,lastclone_list,tmp_server_sids):
            ...
            key = self.eg_port_forward_tbl.make_key(
                ...
                gc.KeyTuple('meta.is_lastclone_for_pktloss',is_lastclone_for_pktloss),
                gc.KeyTuple('hdr.clone_hdr.server_sid',tmp_server_sid)])
            # Update GETRES_UNLOCK as GETRES_FINAL to switchos by cloning
            data = self.eg_port_forward_tbl.make_data(
                [gc.DataTuple('client_sid',self.client_sids[0])],
                'netcacheEgress.clone_get_response_to_client')
            keys.append(key)
            datas.append(data)
            ...
            key = self.eg_port_forward_tbl.make_key(
                [gc.KeyTuple('hdr.op_hdr.optype',GETRES_FINAL),
                gc.KeyTuple('hdr.inswitch_hdr.is_cached',is_cached),
                gc.KeyTuple('meta.is_hot',is_hot),
                gc.KeyTuple('meta.is_latest',is_latest),
                gc.KeyTuple('meta.is_deleted',is_deleted),
            ...
            if is_lastclone_for_pktloss == 0:
                # Forward GETRES_FINAL to client and clone
                data = self.eg_port_forward_tbl.make_data(
                    [gc.DataTuple('client_sid',self.client_sids[0])],
                        'netcacheEgress.forward_get_response_to_client')
                keys.append(key)
                datas.append(data)
            elif is_lastclone_for_pktloss == 1:
                if is_deleted == 1:
                    data = self.eg_port_forward_tbl.make_data(
                        [gc.DataTuple('server_sid',self.server_sids[0]),
                        gc.DataTuple('client_port',12345)],
                        'netcacheEgress.update_getres_final_to_getres_ack_by_mirroring')
                    keys.append(key)
                    datas.append(data)
                # Qingxiu: if we use two servers, we should uncomment the following lines
                # elif is_deleted == 0:
                #     data = self.eg_port_forward_tbl.make_data(
                #         [gc.DataTuple('server_sid',self.server_sids[1]),
                #         gc.DataTuple('client_port',12345)],
                #         'netcacheEgress.update_getres_final_to_getres_ack_by_mirroring')
                #     keys.append(key)
                #     datas.append(data)
            ...
            key = self.eg_port_forward_tbl.make_key(
                [gc.KeyTuple('hdr.op_hdr.optype',GETRES_NO_UNLOCK),
                gc.KeyTuple('hdr.inswitch_hdr.is_cached',is_cached),
                gc.KeyTuple('meta.is_hot',is_hot),
                gc.KeyTuple('meta.is_latest',is_latest),
                gc.KeyTuple('meta.is_deleted',is_deleted),
            ...
            if is_deleted == 1:
                data = self.eg_port_forward_tbl.make_data(
                        [gc.DataTuple('server_sid',self.server_sids[0])],
                        'netcacheEgress.update_getres_no_unlock_to_getres_ack_pkt_loss_by_mirroring')
                keys.append(key)
                datas.append(data)
            # Qingxiu: if we use two physical servers, we should uncomment the following lines
            # elif is_deleted == 0:
            #     data = self.eg_port_forward_tbl.make_data(
            #             [gc.DataTuple('server_sid',self.server_sids[1])],
            #             'netcacheEgress.update_getres_no_unlock_to_getres_ack_pkt_loss_by_mirroring')
    ...
    def configure_update_pktlen_tbl(self):
        ...
        for i in range(int(switch_max_vallen / 8 + 1)):  # i from 0 to 16
            ...
            for tmpoptype in [GETRES_FINAL]:
                ...
                datas.append(self.update_pktlen_tbl.make_data(
                    [gc.DataTuple('udplen_delta', 4)], # add hdr_clone --> clone 2 times
                    'netcacheEgress.add_pktlen'))
            
            for tmpoptype in [GETRES_ACK]:
                keys.append(self.update_pktlen_tbl.make_key([
                    gc.KeyTuple('$MATCH_PRIORITY', 0),
                    gc.KeyTuple('hdr.op_hdr.optype', tmpoptype),
                    gc.KeyTuple('hdr.vallen_hdr.vallen', low = vallen_start, high = vallen_end)
                ...
                datas.append(self.update_pktlen_tbl.make_data(
                    [gc.DataTuple('udplen_delta', 3)], # remove hdr_clone
                    'netcacheEgress.subtract_pktlen'))
    ...
    def configure_update_ipmac_srcport_tbl(self):
        ...
        for tmp_client_physical_idx in range(client_physical_num):
            tmp_devport = self.client_devports[tmp_client_physical_idx]
            data_without_action = [gc.DataTuple('server_port', server_worker_port_start+128)]
            for tmpoptype in [
                # GETRES_UNLOCK,
                # GETRES_FINAL,
                # GETRES_NO_UNLOCK,
                ...
                datas.append(self.update_ipmac_srcport_tbl.make_data(
                    data_without_action,
                    'netcacheEgress.update_ipmac_srcport_server2client_for_getres_final'))
        ...
        for tmp_server_physical_idx in range(server_physical_num):
            ...
            data_without_action_1 = [gc.DataTuple('client_mac', tmp_client_mac),
                gc.DataTuple('server_mac', tmp_server_mac),
                gc.DataTuple('client_ip', tmp_client_ip),
                gc.DataTuple('server_ip', tmp_server_ip),
                gc.DataTuple('client_port', tmp_client_port),
                gc.DataTuple('server_port', server_worker_port_start+128)]
            for tmpoptype in [NETCACHE_VALUEUPDATE_ACK, SETVALID_INSWITCH_ACK, GETRES_ACK, GETRES_PKT_LOSS_ACK]:  # simulate client -> server
                keys.append(self.update_ipmac_srcport_tbl.make_key([
                    gc.KeyTuple('hdr.op_hdr.optype', tmpoptype),
                    gc.KeyTuple('eg_intr_md.egress_port', tmp_devport)
                ]))
            ...
            # for tmpoptype in [GETRES_PKT_LOSS_ACK]:  # simulate client -> server
            #     keys.append(self.update_ipmac_srcport_tbl.make_key([
            #         gc.KeyTuple('hdr.op_hdr.optype', tmpoptype),
            #         gc.KeyTuple('eg_intr_md.egress_port', tmp_devport)
            #     ]))
            #     datas.append(self.update_ipmac_srcport_tbl.make_data(
            #         data_without_action_1,
            #         'netcacheEgress.update_ipmac_srcport_dstport_client2server'))
    ...
    def configure_add_and_remove_value_header_tbl(self):
        ...
        keys = []
        datas = []
        for tmpoptype in [PUTREQ_SEQ, NETCACHE_PUTREQ_SEQ_CACHED, GETRES, GETRES_UNLOCK, GETRES_FINAL, GETRES_ACK, GETRES_PKT_LOSS_ACK]:
    ...
    def configure_recir_for_error_ingress_tbl(self):
        ...
        for tmpoptype in [GETREQ, GETREQ_RECIR, GETRES, PUTREQ]:
            lock_recir_port = recir_ports[self.lock_pipe] # pipe1
            nolock_recir_port = recir_ports[self.nolock_pipe] # pipe0
    ...
    def configure_set_meta_offset_for_lock9_tbl(self):
        keys = []
        datas = []
        for tmpoptype in [GETREQ_RECIR, GETRES, GETRES_UNLOCK]:
    ...
    def runTest(self):
        ...
        print("Configuring access_counter_tbl")
        self.configure_access_counter_tbl()

        print("Configuring check_counter_tbl")
        self.configure_check_counter_tbl()
        ...
        # stage 5
        print("Configuring set_requested_uid_gid_tbl")
        self.configure_set_requested_uid_gid_tbl()

        print("Configuring get_eport_for_getres_for_pkt_loss_tbl")
        self.configure_get_eport_for_getres_for_pkt_loss_tbl()
