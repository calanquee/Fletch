...
# Qingxiu: use 10 for debug
hot_threshold = 10 # for 200 keys under 2 server threads for YCSB-based workload
...
mdtestinipath = "/home/cyh/jzcai/In-Switch-FS-Metadata/netfetch-private/mdtestcpp/config.ini"
mdconfig = configparser.ConfigParser()
with open(mdtestinipath, "r") as f:
    mdconfig.readfp(f)
config = configparser.ConfigParser()
with open(os.path.join(os.path.dirname(this_dir), "config.ini"), "r") as f:
    config.readfp(f)

workload_mode=int(mdconfig.get("global", "mode"))
...
recir_ports = []
recir_ports.append(int(config.get("switch", "recirculate_port_pipe0")))
recir_ports.append(int(config.get("switch", "recirculate_port_pipe1")))
...
GETRES = 0x0009
PRE_GETRES = 0x0019
GETRES_NO_UNLOCK = 0x0029
GETRES_UNLOCK = 0x0039
GETRES_FINAL = 0x0049
GETRES_ACK = 0x0059
GETRES_PKT_LOSS_ACK = 0x0069
# 0b0101
