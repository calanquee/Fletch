...
// op_hdr + vallen&value + shadowtype (0b0001): PUTREQ, PUTREQ_TOUCH_MKDIR, LOADREQ
    ...
    out egress_intrinsic_metadata_t  eg_intr_md)
{
	...
	state parse_udp_dstport {
		...
		transition select(hdr.udp_hdr.dstPort) {
			0x0480 &&& 0xFF80: parse_keydepth; // reserve multiple udp port due to server simulation
			0x0500 &&& 0xFF80: parse_keydepth; // SERVER RES will be in switch
			0x1080 &&& 0xFF80: parse_keydepth; // reserve for server.valueupdateserver in NetCache
			5008: parse_keydepth; // reserve reflector.dp2cpserver_port due to hardware link simulation between switch and switchos
	...
	state parse_udp_srcport {
		transition select(hdr.udp_hdr.srcPort) {
			...
			0x0500 &&& 0xFF80: parse_keydepth; // SERVER RES will be in switch
			0x1080 &&& 0xFF80: parse_keydepth; // reserve for server.valueupdateserver in NetCache
			5009: parse_keydepth; // reserve reflector.cp2dpserver_port due to hardware link simulation between switch and switchos
			default: accept; // traditional packet
		}
	}

	state parse_keydepth{
		packet.extract(hdr.op_hdr);
		transition select(hdr.op_hdr.keydepth){
			0: parse_op;
			default: parse_key1;
		}
	}
	...
	state parse_op {
		transition select(hdr.op_hdr.optype) {
			...
			// GETREQ_RECIR: parse_shadowtype; // for unlock
			1 &&& 0x01: parse_vallen;
			/*2 &&& 0x02: parse_seq;
	...
	state parse_val_len1 {
		...
		transition select(hdr.vallen_hdr.vallen){
			0 &&& 0xFFF8: parse_shadowtype;
			8: parse_shadowtype;
			default: parse_val_len2;
		}
	...
	state parse_val_len2 {
		...
		transition select(hdr.vallen_hdr.vallen){
			8 &&& 0xFFF8: parse_shadowtype;
			16: parse_shadowtype;
			default: parse_val_len3;
		}
	...
	state parse_val_len3 {
		...
		transition select(hdr.vallen_hdr.vallen){
			16 &&& 0xFFF8: parse_shadowtype;
			24: parse_shadowtype;
			default: parse_val_len4;
		}
	...
	state parse_val_len4 {
		...
		transition select(hdr.vallen_hdr.vallen){
			24 &&& 0xFFF8: parse_shadowtype;
			32: parse_shadowtype;
			default: parse_val_len5;
		}
	...
	state parse_val_len5 {
		...
		transition select(hdr.vallen_hdr.vallen){
			32 &&& 0xFFF8: parse_shadowtype;
			40: parse_shadowtype;
			default: parse_val_len6;
		}
	...
	state parse_val_len6 {
		...
		transition select(hdr.vallen_hdr.vallen){
			40 &&& 0xFFF8: parse_shadowtype;
			48: parse_shadowtype;
			default: parse_val_len7;
		}
	...
	state parse_val_len7 {
		...
		transition select(hdr.vallen_hdr.vallen){
			48 &&& 0xFFF8: parse_shadowtype;
			56: parse_shadowtype;
			default: parse_val_len8;
		}
	...
	state parse_val_len8 {
		...
		transition select(hdr.vallen_hdr.vallen){
			56 &&& 0xFFF8: parse_shadowtype;
			64: parse_shadowtype;
			default: parse_val_len9;
		}		
	...
	state parse_val_len9 {
		...
		transition select(hdr.vallen_hdr.vallen){
			64 &&& 0xFFF8: parse_shadowtype;
			72: parse_shadowtype;
			default: parse_val_len10;
		}		
	}
	...
	state parse_val_len10 {
		...
		transition select(hdr.vallen_hdr.vallen){
			72 &&& 0xFFF8: parse_shadowtype;
			80: parse_shadowtype;
			default: parse_val_len11;
		}	
	}
	...
	state parse_val_len11 {
		...
		transition select(hdr.vallen_hdr.vallen){
			80 &&& 0xFFF8: parse_shadowtype;
			88: parse_shadowtype;
			default: parse_val_len12;
		}	
	}
	...
	state parse_val_len12 {
		...
		transition select(hdr.vallen_hdr.vallen){
			88 &&& 0xFFF8: parse_shadowtype;
			96: parse_shadowtype;
			default: parse_val_len13;
		}	
	}
	...
	state parse_val_len13 {
		...
		transition select(hdr.vallen_hdr.vallen){
			96 &&& 0xFFF8: parse_shadowtype;
			104: parse_shadowtype;
			default: parse_val_len14;
		}	
	}
	...
	state parse_val_len14 {
		...
		transition select(hdr.vallen_hdr.vallen){
			104 &&& 0xFFF8: parse_shadowtype;
			112: parse_shadowtype;
			default: parse_val_len15;
		}	
	}
	...
	state parse_val_len15 {
		...
		transition select(hdr.vallen_hdr.vallen){
			112 &&& 0xFFF8: parse_shadowtype;
			120: parse_shadowtype;
			default: parse_val_len16;
		}	
	}
	...
	state parse_stat {
		...
		transition select(hdr.shadowtype_hdr.shadowtype) {
			...
			GETRES_FINAL: parse_clone;
			default: parse_token; // CACHE_POP_INSWITCH
			//default: parse_debug;
		}
...
control netcacheDeparser(packet_out packet,
    ...
    apply {
		...
		if (eg_dprsr_md.mirror_type == MIRROR_TYPE_E2E) {
            // mirror.emit<mirror_h>(meta.egr_mir_ses);
