...
#define CURMETHOD_ID NETFETCH_ID
...
// new structure for rm/rmdir/rename
struct key_op_path_t
{
    netreach_key_t key;
    int8_t failure_flag; // 1: failure; 0 :success; use int8_t for consistency
    uint16_t path1_length;
    char path1[MAX_PATH_LENGTH];

    key_op_path_t()
        : key(), failure_flag(0), path1_length(0)
    {
        path1[0] = '\0';
    }

    key_op_path_t(netreach_key_t k, int8_t failure_flag = 0, uint16_t path1_length = 0, const char *p1 = "")
        : key(k), failure_flag(failure_flag), path1_length(path1_length)
    {
        if (path1_length > 0 && path1_length < MAX_PATH_LENGTH)
        {
            std::strncpy(path1, p1, path1_length);
            path1[path1_length] = '\0';
        }
        else
        {
            path1[0] = '\0';
    ...
    bool operator<(const key_op_path_t &other) const
    {
        return key < other.key;
    }
};

size_t netfetch_delete_serialize_to_buffer(const key_op_path_t *obj, char *buffer)
{
    optype_t tmp_optype = static_cast<optype_t>(packet_type_t::DELETE_SWITCHOS);
    optype_t bigendian_optype = htons(tmp_optype);
    size_t tmp_offset = 0;
    std::memcpy(buffer + tmp_offset, &bigendian_optype, sizeof(bigendian_optype));
    keydepth_t keydepth = 0x0000; // used to indicate that this request is a delete request
    keydepth_t bigendian_keydepth = htons(keydepth);
    tmp_offset += sizeof(optype_t);
    std::memcpy(buffer + tmp_offset, &bigendian_keydepth, sizeof(bigendian_keydepth));
    tmp_offset += sizeof(keydepth_t);
    std::memcpy(buffer + tmp_offset, &obj->key, sizeof(obj->key));
    tmp_offset += sizeof(netreach_key_t);
    buffer[tmp_offset] = obj->failure_flag;
    tmp_offset += sizeof(obj->failure_flag);
    uint16_t path1_len_net = htons(obj->path1_length);
    std::memcpy(buffer + tmp_offset, &path1_len_net, sizeof(path1_len_net));
    tmp_offset += sizeof(path1_len_net);
    std::memcpy(buffer + tmp_offset, obj->path1, obj->path1_length);
    tmp_offset += obj->path1_length;
    return tmp_offset;
}

key_op_path_t *netfetch_delete_deserialize_from_buffer(const char *buffer, size_t buffer_size)
{
    key_op_path_t *obj = new key_op_path_t();
    size_t tmp_offset = sizeof(optype_t) + sizeof(keydepth_t);
    if (buffer_size < sizeof(obj->key))
    {
        throw std::runtime_error("Buffer too small for key");
    }
    std::memcpy(&obj->key, buffer + tmp_offset, sizeof(netreach_key_t));
    tmp_offset += sizeof(netreach_key_t);
    obj->failure_flag = buffer[tmp_offset];
    tmp_offset += sizeof(obj->failure_flag);
    uint16_t path1_len_net;
    std::memcpy(&path1_len_net, buffer + tmp_offset, sizeof(path1_len_net));
    obj->path1_length = ntohs(path1_len_net);
    tmp_offset += sizeof(obj->path1_length);
    std::memcpy(obj->path1, buffer + tmp_offset, obj->path1_length);
    obj->path1[obj->path1_length] = '\0';
    return obj;
}

inline std::vector<char> reverse_key_for_getres_pkt_loss(char *getreq_buf, int recv_size, int keydepth, int counter)
{
    std::vector<char> reversed_key((keydepth + 1) * sizeof(netreach_key_t));
    int tmp_offset = sizeof(optype_t) + sizeof(keydepth_t);

    for (int i = 0; i < keydepth; ++i)
    {
        int start_pos = tmp_offset + (keydepth - 1 - i) * sizeof(netreach_key_t);
        std::copy(getreq_buf + start_pos, getreq_buf + start_pos + sizeof(netreach_key_t),
                  reversed_key.begin() + i * sizeof(netreach_key_t));
    }

    // set the last key's keyhihihi as counter
    // change last two char of reversed_key to counter
    reversed_key.at(reversed_key.size() - 1) = static_cast<char>(static_cast<uint8_t>(counter));
    reversed_key.at(reversed_key.size() - 2) = 0x00; 
    return reversed_key;
}

inline std::vector<char> reverse_key_for_getres(char *getreq_buf, int recv_size, int keydepth)
{
    std::vector<char> reversed_key(keydepth * sizeof(netreach_key_t));
    int tmp_offset = sizeof(optype_t) + sizeof(keydepth_t);
...
int file_metadata_length = 0;
int dir_metadata_length = 0;
int path_resolution_offset = 0;
...
short client_port1_for_handling_updatecache = 0;
short client_port2_for_handling_updatecache = 0;
...
short server_valuedeleteserver_port_start = -1;
short transaction_loadfinishserver_port = -1;
int file_parser_offset = 0;
int server_thread_pool_size = 0;
int delete_server_number = 0;
...
std::vector<const char *> server_ips_for_invalidation_list;
std::vector<const char *> server_ports_for_invalidation_list;
std::vector<const char *> another_server_ips_for_invalidation_list;
...
short switchos_server_eviction_port = -1;
uint32_t cache_lookup_table_size = 0;
const char *root_path_for_fs_tree = nullptr;

// reflector
const char *reflector_ip_for_switchos = nullptr;
short reflector_dp2cpserver_port = -1;
short reflector_cp2dpserver_port = -1;
// Qingxiu: add a port for loading freq.
short reflector_cp2dpserver_loadfreq_port = -1;
...
inline void parse_ini(const char *config_file)
{
    ...
    file_metadata_length = ini.get_file_metadata_length();
    dir_metadata_length = ini.get_dir_metadata_length();
    path_resolution_offset = ini.get_path_resolution_offset();
    ...
    COUT_VAR(path_resolution_offset);
    printf("\n");
    ...
    // client_port1_for_handling_updatecache = ini.get_client_port1_for_handling_updatecache();
    // client_port2_for_handling_updatecache = ini.get_client_port2_for_handling_updatecache();
    ...
    // COUT_VAR(client_port1_for_handling_updatecache);
    // COUT_VAR(client_port2_for_handling_updatecache);
    printf("\n");
    ...
    server_valuedeleteserver_port_start = ini.get_server_valuedeleteserver_port_start();
    transaction_loadfinishserver_port = ini.get_transaction_loadfinishserver_port();
    file_parser_offset = ini.get_file_parser_offset();
    server_thread_pool_size = ini.get_server_thread_pool_size();
    delete_server_number = ini.get_delete_server_number();
    ...
    COUT_VAR(server_valuedeleteserver_port_start);
    COUT_VAR(transaction_loadfinishserver_port);
    COUT_VAR(server_thread_pool_size);
    COUT_VAR(delete_server_number);
    printf("\n");
    ...
    for (size_t server_physical_idx = 0; server_physical_idx < server_physical_num; server_physical_idx++)
    {
        ...
        server_ips_for_invalidation_list.push_back(ini.get_server_ip_for_invalidation_list(server_physical_idx));
        server_ports_for_invalidation_list.push_back(ini.get_server_port_for_invalidation_list(server_physical_idx));
        another_server_ips_for_invalidation_list.push_back(ini.get_another_server_ip_for_invalidation_list(server_physical_idx));
        ...
        printf("server_ips_for_invalidation_list[%d]: %s\n", server_physical_idx, server_ips_for_invalidation_list[server_physical_idx]);
        printf("server_ports_for_invalidation_list[%d]: %s\n", server_physical_idx, server_ports_for_invalidation_list[server_physical_idx]);
        printf("another_server_ips_for_invalidation_list[%d]: %s\n", server_physical_idx, another_server_ips_for_invalidation_list[server_physical_idx]);
    }
    ...
    root_path_for_fs_tree = ini.get_root_path_for_fs_tree();
    ...
    switchos_server_eviction_port = ini.get_switchos_server_eviction_port();
    cache_lookup_table_size = ini.get_cache_lookup_table_size();
    ...
    printf("root_path_for_fs_tree: %s\n", root_path_for_fs_tree);
    ...
    COUT_VAR(switchos_server_eviction_port);
    ...
    // Qingxiu: add a port for loading freq.
    reflector_cp2dpserver_loadfreq_port = ini.get_reflector_cp2dpserver_loadfreq_port();
    ...
    // Qingxiu: add a port for loading freq.
    COUT_VAR(reflector_cp2dpserver_loadfreq_port);
    printf("\n");
